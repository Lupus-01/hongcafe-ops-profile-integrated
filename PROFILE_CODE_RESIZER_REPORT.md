...
- 생성·편집·이미지 저장 66/35px, 줄 간격 1.7 및 사이트 출력 26/16px 기준 유지. `script.js`, `style.css`, `profile-site-preview.js`는 수정하지 않았고 작업 전후 SHA-256이 동일하다. index.html은 코드 변환기 안내 문구 2곳만 수정했다.
- 변환기는 기존 목록 기호의 텍스트·속성·요소를 보존하고 CSS만 수정한다. 빈 기호 span은 CSS 도형으로 표시하고, 기호 요소가 없는 목록은 list-style로 표시한다. 기호 요소 추가·교체·삭제 로직은 제거했다.
- 기호가 중복되거나 기호 안에 예상하지 못한 내용이 있으면 원문을 고치지 않고 변환을 중단한다.
- `verifyStyleOnlySource()`는 결과의 style 변경분만 원래대로 되돌려 원본 문자열과 정확히 비교한다. 문구·기호·URL·링크·HTML 구조·속성·공백·줄바꿈·BOM·주석이 바뀌면 출력을 차단한다. DOM 비교에서도 기호 제거 예외를 없앴다. 지정된 CSS만 변경했는지 재계산하는 기존 검사도 유지한다.
- 수정 파일: profile-code-resizer.js, profile-code-resizer-ui.js, index.html, profile-code-resizer.test.mjs, profile-code-resizer-browser.test.mjs, profile-site-layout-browser.test.mjs 및 이 보고서 총 7개. package.json의 기존 사용자 변경과 미추적 항목은 보존했다.

검증: npm run check 통과. 코드 변환·파일 읽기·Word API·사이트 출력·타이포·히스토리 관련 테스트 37/37 통과. Chrome 브라우저 검사 3개 통과(원문 보존 및 복사·저장, 사이트 레이아웃, 기존 제작 미리보기·이미지 저장 설정). 레이아웃은 3유형 × 4폭 × 4경로(현재 생성 출력, 빈 기호, 문자 기호, 기호 요소 없음) = 48조합을 확인했다. 샌드박스 Chrome은 권한 오류로 실패했고 승인된 권한 확장으로 검증했다. 샘플 기반 로컬 검사이며 실제 운영 화면 검수나 PNG 품질 검사는 아니다. git diff --check 및 전체 diff 검토 완료.

보존 파일 SHA-256:

- script.js: `7E9FC60472D3ABAA3C5D69BE756660BE7E22B8D19CF6F180A7D4D8FFE6B553CA`
- style.css: `6741A7352F8F7A3582AF9538E6DF6B533159FEBCC6ED08E6F96C5665C32C2B53`
- profile-site-preview.js: `B7D29ADFA1786FCE7F15557CC94EBD90D7CF6ABDDADC45C1AC4A7E8FDC11A5EA`

현재 미커밋이며 커밋·푸시·운영 반영·PM2 조작은 수행하지 않았다. 권장 커밋 메시지: `수정: 프로필 코드 변환을 CSS 변경으로 제한하고 원문 보존 검증 강화`.
