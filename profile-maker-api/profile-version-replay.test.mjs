...
test('v7/v11 through v9/v14 exact requests replay across reference changes without new jobs or billing', () => {
    ...
    for (const [oldText, oldVisual] of [['profile-copy-v7-concrete-editorial-direction', 'profile-visual-v11-photographic-direction'], ['profile-copy-v8-source-first', 'profile-visual-v12-macro-balance'], ['profile-copy-v9-expanded-editorial', 'profile-visual-v13-expanded-scenes'], ['profile-copy-v9-expanded-editorial', 'profile-visual-v14-independent-shots']])
    for (const kind of ['direct', 'document']) for (const generateImageRequested of [false, true]) {
        const input = { profileTextPromptVersion: 'profile-copy-v9-expanded-editorial', visualVariationVersion: generateImageRequested ? 'profile-visual-v15-overhead-accessories' : 'none', referenceInfluenceVersion: 'profile-reference-v3-material-only', referenceText: 'unchanged source', referenceDigests: ['reference-image'], generateImageRequested };
        const previous = createProfileJobFingerprint({ kind, ...input, profileTextPromptVersion: oldText, visualVariationVersion: generateImageRequested ? oldVisual : 'none', referenceInfluenceVersion: oldVisual === 'profile-visual-v14-independent-shots' ? 'profile-reference-v3-material-only' : 'profile-reference-v2-strong-priority' });
...
test('v14 request keys and expired receipts survive a cold restart into v15 without billing', (t) => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'hongcafe-v15-replay-'));
    t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
    const source = fs.readFileSync(new URL('./server.mjs', import.meta.url), 'utf8');
    const functionSource = source.slice(source.indexOf('function submitProfileJob('), source.indexOf('const profileRecoveryStartedAt')).trim();
    for (const expired of [false, true]) for (const kind of ['direct', 'document']) {
        const campaignId = `${kind}-${expired}`;
        const oldInput = { profileTextPromptVersion: 'profile-copy-v9-expanded-editorial', visualVariationVersion: 'profile-visual-v14-independent-shots', referenceInfluenceVersion: 'profile-reference-v3-material-only', referenceText: 'same source', referenceDigests: ['same-reference'], generateImageRequested: true };
        ...
        const options = { kind, fingerprintInput: { ...oldInput, visualVariationVersion: 'profile-visual-v15-overhead-accessories', referenceInfluenceVersion: 'profile-reference-v3-material-only' }, input: {} };
