...
test('category copy prompts create a new fingerprint while preserving safe legacy image reuse', () => {
    assert.match(serverSource, /const PROFILE_TEXT_PROMPT_VERSION = 'profile-copy-v7-concrete-editorial-direction';/);
    ...
    assert.match(serverSource, /PREVIOUS_PROFILE_TEXT_PROMPT_VERSIONS = \['profile-copy-v6-cross-campaign-history', 'profile-copy-v5-generation-sequence', 'profile-copy-v4-category-language-separation', 'profile-copy-v3-category-combinations', 'profile-copy-v2-two-line-headline', 'legacy'\]/);
